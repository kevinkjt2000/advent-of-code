...
schedules = {}
event_regex = re.compile("\[(?P<timestamp>.*)\] (?P<event>.*)")
guard_regex = re.compile("Guard #([0-9]*).*")
time_regex = re.compile(".*00:(?P<minute>[0-9][0-9])")
guard_number = None
for line in lines:
    ...
    if "Guard" in event:
        m = guard_regex.search(event)
        guard_number = int(m.group(1))
        if guard_number not in schedules:
            schedules[guard_number] = []
    elif "falls" in event:
        m = time_regex.search(timestamp)
        schedules[guard_number].append(int(m.group("minute")))
    else:
        m = time_regex.search(timestamp)
        start = schedules[guard_number][-1]
        schedules[guard_number][-1] = range(start, int(m.group("minute")))
...
d = defaultdict(int)
for r in schedules[max_key]:
    for x in list(r):
        d[x] += 1
maxi = max(d, key=d.get)
print(max_key, maxi, max_key * maxi)

for key in totals:
    if schedules[key] != []:
        d = defaultdict(int)
        for r in schedules[key]:
            for x in list(r):
                d[x] += 1
        maxi = max(d.items(), key=operator.itemgetter(1))[0]
        print(key, maxi, d[maxi])
